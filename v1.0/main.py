...
def inputCardInex(nowPlayer):
    playCardIndex = -1
    while not (0 <= playCardIndex and playCardIndex <= len(nowPlayer.returnAllCards())):
        playCardIndex = getInput(int, msg="Pick a card to play : ")-1
...
if __name__ == "__main__":
    ...
    while not game.isPlayerWin():
        ...
        if game.returnTurn() < playerNumber:
            printCards(allCards)
            playCardIndex = inputCardInex(nowPlayer)
